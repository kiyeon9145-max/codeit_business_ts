...
export class MemoScreen {
  ...
  async showAllMemosUI(credential: string) {
    const memos = await this._memoService.getMyMemos(credential);
  ...
  async showCreateMemoUI(credential: string) {
    const title = prompt("제목: ");
    const content = prompt("내용: ");

    const isCreated = await this._memoService.createMemo(
      credential,
      title,
      content,
    );
